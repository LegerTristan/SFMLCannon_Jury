...
{
	const float _radRot = (movingPartSprite->getRotation() - ROTATION_OFFSET) * RADIAN_CONVERSION;
	const float _cosPos = cosf(_radRot);
	const float _sinPos = sinf(_radRot);
